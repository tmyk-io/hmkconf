...
> **Repos:** Configurator work lives in `hmkconf`. Firmware work lives in `libhmk` (see sibling plan of the same name). Protocol and on-wire structs in this document are the contract both sides must match.  
